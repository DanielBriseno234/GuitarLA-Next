...
export default function Carrito({carrito, actualizarCantidad, eliminarProducto}) {
    ...
    useEffect( () => {
        const totalAPagar = carrito.reduce( (total, producto) => total + (producto.cantidad * producto.precio), 0 );
                    ...
                    { carrito.length === 0 ? "Carrito Vacío" : (
                        carrito.map( producto => (
                            <div  key={producto.id} className={styles.producto}>
                                <div>
                                    <Image src={producto.imagen} width={280} height={450} alt={producto.nombre} />
                                </div>
                                <div>
